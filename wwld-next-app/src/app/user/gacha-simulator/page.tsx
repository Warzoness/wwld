...
import styles from '@/app/admin/gacha-simulator/GachaSimulator.module.css'
    ...
    <div className={styles.page}>
      <div className={styles.wrap}>
            ...
            {banners.map(b => {
              ...
              return (
                <div key={b.id} className={styles.row}>
                  ...
                  <div className={styles.col}>{b.bannerType == "CHARACTER" ? "Nhân vật" : "Vũ khí"}</div>
                  ...
                  <div className={styles.col}>
                    <div className={styles.actions} style={{ justifyContent: "start" }}>
        ...
        <section className='m-4'>
            <h3>Banner Hiện tại : <strong> {currentBanner?.bannerName ?? "Vui lòng chọn banner trước"}</strong></h3>
        </section>
        ...
        <section className={styles.board}>
          ...
          <div className={styles.stage}>
            {results.length === 0 ? (
              <div className={styles.empty}><div className={styles.orb} /><p className={styles.muted}>Nhấp để tiếp tục</p></div>
            ) : (
              <>
        ...
        <section className={styles.history}>
          <div className={styles.historyHead}>
            ...
            <div className={styles.pager}>
              <button className={styles.btn} disabled={safePage <= 1} onClick={() => setHistoryPage(p => Math.max(1, p - 1))}>Trước</button>
              <span className={styles.pageInfo}>{safePage}/{totalPages}</span>
              <button className={styles.btn} disabled={safePage >= totalPages} onClick={() => setHistoryPage(p => Math.min(totalPages, p + 1))}>Sau</button>
            </div>
          </div>
          <ul className={styles.historyList}>
            ...
            {state.history.length === 0 && <li className={styles.muted}>Chưa có lượt roll nào được thực hiện.</li>}
          </ul>
        </section>
      </div>
    </div>
