...
import { deleteNoteData, fetchListNote } from "@/lib/services/noteListService";
import NoteViewModal from "@/components/modals/NoteListModal/NoteViewModal";
import NoteUpsertDeleteModal from "@/components/modals/NoteListModal/NoteUpsertDeleteModal";
...
export default function NoteListPage() {
    ...
    const handleSaved = (saved: NoteData) => {
        if (mode === "create") {
            setAllNotes((list) => [saved, ...list]); // prepend
        } else {
            setAllNotes((list) => list.map((n) => (n.id === saved.id ? saved : n)));
        }
    };

    const handleDeleted = (id: number) => {
        setAllNotes((list) => list.filter((n) => n.id !== id));
    };
    ...
    const handleDelete = async (id: number) => {
        if (!confirm("Xác nhận xoá note này?")) return;
        try {
            await deleteNoteData(id);
            setAllNotes((list) => list.filter((x) => x.id !== id));
            const after = filtered.length - 1;
            const afterTotalPages = Math.max(1, Math.ceil(after / Math.max(1, pageSize)));
            if (pageIndex >= afterTotalPages) {
                setPageIndex(Math.max(0, afterTotalPages - 1));
            }
        } catch {
            alert("Xoá thất bại!");
        }
        ...
        <div className={styles.wrapper}>
            ...
            <div className={styles.toolbar}>
                ...
                <div className={styles.actions}>
                    <button onClick={openCreate} className={styles.cta}>+ Thêm note</button>
                </div>
            </div>
                                    ...
                                    <tr key={n.id}>
                                        ...
                                        <td className={styles.td}>
                                            <div className={styles.rowActions}>
                                                ...
                                                <button className={styles.btn} onClick={() => openEdit(n)}>
                                                    Sửa/Xoá
                                                </button>
                                            </div>
                                        </td>
