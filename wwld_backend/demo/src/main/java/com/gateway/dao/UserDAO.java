...
public interface UserDAO extends JpaRepository<User, Long> {
    ...
    @Query("SELECT new com.gateway.dto.UserDTO(" +
            "ur.id, ur.fullname,ur.username, ur.email, ur.role, ur.status," +
            "ur.createdAt, ur.updatedAt, ur.hashpassword" +
            ") FROM User ur " +
            "WHERE ( ur.id = :user_id OR :user_id IS NULL ) " +
            "AND (ur.fullname LIKE concat('%',:username,'%') OR :username = '' OR :username IS NULL )" +
            " ")
    ...
    UserDTO findUserByEmail(@Param("username") String username);
}
